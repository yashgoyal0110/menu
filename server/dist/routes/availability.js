...
async function ownedStore(storeId, userId) {
    return db.store.findFirst({ where: { id: storeId, userId, deletedAt: null } });
}
const availabilityInclude = {
    businessHours: { orderBy: { dayOfWeek: 'asc' } },
    availabilityResources: { where: { active: true }, orderBy: { createdAt: 'asc' } },
...
router.post('/stores/:storeId/resources', async (req, res, next) => {
    try {
        const store = await ownedStore(req.params.storeId, req.auth?.userId);
        if (!store) {
            res.status(404).json({ error: 'Location not found.' });
...
router.patch('/stores/:storeId/resources/:resourceId', async (req, res, next) => {
    try {
        const store = await ownedStore(req.params.storeId, req.auth?.userId);
        if (!store) {
            res.status(404).json({ error: 'Location not found.' });
        ...
        if (!existing) {
            res.status(404).json({ error: 'Capacity pool not found.' });
            return;
        }
        ...
        if (availableNow > totalCapacity) {
            res.status(400).json({ error: 'Available capacity cannot exceed total capacity.' });
            return;
        }
...
router.delete('/stores/:storeId/resources/:resourceId', async (req, res, next) => {
    try {
        const store = await ownedStore(req.params.storeId, req.auth?.userId);
        if (!store) {
            res.status(404).json({ error: 'Location not found.' });
...
router.post('/stores/:storeId/exceptions', async (req, res, next) => {
    try {
        const store = await ownedStore(req.params.storeId, req.auth?.userId);
        if (!store) {
            res.status(404).json({ error: 'Location not found.' });
...
router.delete('/stores/:storeId/exceptions/:exceptionId', async (req, res, next) => {
    try {
        const store = await ownedStore(req.params.storeId, req.auth?.userId);
        if (!store) {
            res.status(404).json({ error: 'Location not found.' });
