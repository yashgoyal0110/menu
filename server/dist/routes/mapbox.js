...
router.get('/suggest', async (req, res, next) => {
    try {
        ...
        if (!accessToken) {
            res.status(503).json({
                error: 'Mapbox is not configured. Set MAPBOX_TOKEN on the server to enable address search.',
            });
            return;
        }
...
router.get('/retrieve', async (req, res, next) => {
    try {
        ...
        const url = new URL(`${SEARCHBOX_BASE}/retrieve/${encodeURIComponent(id)}`);
        url.searchParams.set('access_token', accessToken);
        url.searchParams.set('session_token', sessionToken);
        url.searchParams.set('language', 'en');
        const response = await fetch(url);
        ...
        const cityName = context?.place?.name ??
