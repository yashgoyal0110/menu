...
type Exception = { date: Date; isClosed: boolean; openMinute: number | null; closeMinute: number | null; note: string | null }
type Resource = { id: string; name: string; singularLabel: string; totalCapacity: number; availableNow: number; updatedAt: Date }
...
type AvailabilityStore = {
  ...
  availabilityResources: Resource[]
}
...
export function availabilitySnapshot(store: AvailabilityStore, now = new Date()) {
  if (!store.availabilityEnabled) {
    return { enabled: false, status: 'NOT_CONFIGURED' as const, timezone: store.timezone, resources: [] }
  }
  ...
  return {
    enabled: true,
    status: !open ? 'CLOSED' as const : full ? 'FULL' as const : 'AVAILABLE' as const,
